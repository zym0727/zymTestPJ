        ...
        value: function initServer(silent, query, url) {
          ...
          var request = $.extend({}, Utils.calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            success: function success(_res) {
              if(_this9.options.pagination&&_res.total&&!_res.rows.length){//总记录数大于0,但当前页记录数为0,则此时页码超过了最大页码
                  _this9.options.pageNumber = Math.ceil(_res.total/_this9.options.pageSize);//最后一页(总页数)
                  _this9.initServer();
                  return;
              }
